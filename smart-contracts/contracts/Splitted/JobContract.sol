...
contract JobContract is ReentrancyGuard {
    ...
    function CheckPersonHired(uint _nrJob) onlyJobExist(_nrJob) onlyPerson external view returns(bool) {
        Job storage job = jobs[_nrJob];
    ...
    function ShowIfHireQuestion(uint _nrJob) onlyJobExist(_nrJob) onlyPerson external view returns(bool){
        Job storage job = jobs[_nrJob];
