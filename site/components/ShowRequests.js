            ...
            <>
                ...
                <div className="sm:grid md:grid-cols-2 lg:grid-cols-3 sm:w-full">
                    {requests.map((item,k) => {
                        ...
                        //request.title, request.description, request.workingAddress, request.hourInit,request.hourFinish,request.dateFrom, request.dateTo,request.value)
                        
                        let nrRequest = item.nr;
                        let owner= item.value[0];
                        let destination=item.value[1];
                        let title = HexToDec(item.value[2]);
                        let description = item.value[3];
                        let workingAddress = item.value[4];

                        console.log(destination,title,description,workingAddress )
                        ...
                        let dateFrom = new Date(HexToDec(item.value[7])*1000);

                        let dateTo = new Date(HexToDec(item.value[8])*1000);
                        //console.log(item.value[9]);
                        //let value = Moralis.Units.FromWei(item.value[9]);
